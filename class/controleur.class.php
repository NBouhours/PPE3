...
class controleur {
	...
	public function retourne_formulaire_login() 
	{
		...
		<article >
			...
			<form id="login" method="post" class="login">
				...
				<input type="radio" name="rblogin" id="rba" value="rba" checked required/>Administrateur</br></br>
	...
	public function retourne_formulaire_enfant($type,$idenfant) 
	{
		...
		<article >
			...
			<form id="formenfant" method="post" >
				...
				<input type="hidden" id="id_enfant" value='.$id_enfant.'> 
			...
			<div id="modal" >
				<h1>Informations !</h1>
				<div id="dialog1" ></div>
				<a class="no" onclick="hd();" href= ajout_enfant.php >OK</a>
			</div>
		<article>
			<script>
				$("#modal").hide();
	...
	public function retourne_formulaire_famille($type,$idfamille="")
	{
			...
			<article >
				...
				<form id="formfamille" method="post" >';
					if ($type == 'Ajout') {
						$vmp=$this->genererMDP();
						$form = $form . '
							<div >Identifiant : <input type="text" name="identifiant" id="identifiant" placeholder="Identifiant famille" value="' . $identifiant . '" required/></br>
		...
		$form = $form . ' 
					...
					<input type="hidden" id="id_famille" value="' . $idfamille . '"/>
					...
					';
					if (($type == 'Modif') && $_SESSION['type']=='admin')
					$form .= '<input id="submits" type="button" name="send" class="button" value="Supprimer" />';
				...
				<div  id="modal" >
					<h1>Informations !</h1>
					<div id="dialog1" ></div>
					<a class="no" onclick="hd()" href= modif_famille.php>OK</a>
				</div>
			<article >
				<script>
					...
					$.ajax(
			    		{
			        		...
			        		type: "POST",
			        		url: "ajax/valide_supp_famille.php",
			        		dataType: "json",
							encode : true,
