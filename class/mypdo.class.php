...
class mypdo extends PDO{
    ...
    public function supp_enfant_famille($tab)
    {
        ...
        $errors         = array();      
        $data           = array();   
        $requete='DELETE FROM enfant WHERE id_enfant="'.$tab['id_enfant'].'";';
    ...
    public function liste_famille()
    {
     ...
     public function supp_famille($tab)
    {
        
        $errors         = array();      
        $data           = array();   
        $requete1 = 'SELECT COUNT(*) FROM enfant WHERE id_famille = '.$tab['id_famille'].' ;';
        
         $nblignes=$this->connexion -> query($requete1);
         $count = $nblignes->fetch(PDO::FETCH_ASSOC);
         $counts = $count['COUNT(*)'];
         ...
         if ($counts == 0)
        {
            $requete='DELETE FROM famille WHERE id_famille='.$tab['id_famille'].';';
            $nbL=$this->connexion -> exec($requete);

            if ($nbL !=1){
                $errors['requete']='Pbs suppression famille :'.$requete; }
             ...
             if ( ! empty($errors)) {
                ...
                $data['success'] = true;
                $data['message'] = 'famille supprimée !';
            }
        }
        else{
            $data['supprimer'] = true;
            $data['success'] = true;
            $data['message'] = 'la famille a encore des enfants!';
        }
